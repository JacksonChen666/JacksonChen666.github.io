...
// TODO: leaderboard and submit score
...
function set() {
	var a = parseInt(prompt("What do you want to set to?", time));
	time = a;
	time++;
	spacebarGame();
}
...
function load(mobile = false){
	if (mobile == false) {
		document.getElementById('mobileCount').parentNode.removeChild(document.getElementById('mobileCount'));
		console.log("Loading Desktop Version");
	} else {
		console.log("Loading Mobile Version");
	}
	spacebarGame();
	keyDetection(32); // https://keycode.info/
	return mobile;
}
